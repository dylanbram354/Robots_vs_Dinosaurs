...
class Robot:
    ...
    def choose_weapon(self):
        option_1 = Weapon('laser gun', 20)
        option_2 = Weapon('rocket launcher', 30)
        option_3 = Weapon('sword', 25)
        options = [option_1, option_2, option_3]
        print(f"Choose this robot's weapon:")
        i = 1
        while i <= len(options):
            print(f"Option {i}: {options[i-1].type}, attack power {options[i-1].attack_power}")
            i += 1
